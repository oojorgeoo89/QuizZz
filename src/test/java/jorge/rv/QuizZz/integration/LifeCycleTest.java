...
public class LifeCycleTest {
	...
	private void test_quizPublishing() throws Exception {
		...
		mvc.perform(post(QuizController.ROOT_MAPPING).param(QUIZ_NAME_KEY, QUIZ_NAME_1)
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isCreated());

		// Try to publish a Quiz without questions
		mvc.perform(post(QuizController.ROOT_MAPPING + "/3/publish").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isForbidden());

		// Create a question
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(3)).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isCreated());
		...
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(5))
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isCreated());
	...
	private void test_fetchValidQuestions() throws Exception {
		...
		mvc.perform(get(QuizController.ROOT_MAPPING + "/1/questions").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk()).andExpect(jsonPath("$", Matchers.hasSize(3)));

		// Get valid questions from Quiz 1
		mvc.perform(
				get(QuizController.ROOT_MAPPING + "/1/questions?onlyValid=true").with(httpBasic(EMAIL_1, PASSWORD_1)))
	...
	private void test_deleteUsers() throws Exception {
		...
		mvc.perform(delete(UserController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isUnauthorized());

		// Deleting an User
		mvc.perform(delete(UserController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isOk());
	...
	private void test_deleteQuizzes() throws Exception {
		...
		mvc.perform(delete(QuizController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isUnauthorized());

		// Deleting a Quiz
		mvc.perform(delete(QuizController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isOk());
	...
	private void test_deleteQuestions() throws Exception {
		...
		mvc.perform(delete(QuestionController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());

		// Deleting a Question
		mvc.perform(delete(QuestionController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk());
	...
	private void test_deleteAnswers() throws Exception {
		...
		mvc.perform(delete(AnswerController.ROOT_MAPPING + "/1").with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());

		// Deleting a correct Answer
		mvc.perform(delete(AnswerController.ROOT_MAPPING + "/2").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isForbidden());

		// Deleting a correct Answer
		mvc.perform(delete(AnswerController.ROOT_MAPPING + "/1").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk());
	}

	private void test_playQuiz() throws Exception {
		// Send results for non existing quiz
		mvc.perform(post(QuizController.ROOT_MAPPING + "/55/submitAnswers").contentType(MediaType.APPLICATION_JSON)
				.content("[ { \"question\": 1, \"selectedAnswer\": 1 }]")).andExpect(status().isNotFound());

		// Send results with a missing question
		mvc.perform(post(QuizController.ROOT_MAPPING + "/1/submitAnswers").contentType(MediaType.APPLICATION_JSON)
				.content("[ { \"question\": 1, \"selectedAnswer\": 1 }]")).andExpect(status().isBadRequest());

		// Send results with an invalid answer
		mvc.perform(post(QuizController.ROOT_MAPPING + "/1/submitAnswers").contentType(MediaType.APPLICATION_JSON)
				.content("[ " + "{ \"question\": 1, \"selectedAnswer\": 2 },"
						+ "{ \"question\": 2, \"selectedAnswer\": 55 }" + "]"))
				.andExpect(status().isOk()).andExpect(jsonPath("$." + TOTAL_QUESTIONS_TEXT, Matchers.equalTo(2)))
		...
		mvc.perform(post(QuizController.ROOT_MAPPING + "/1/submitAnswers").contentType(MediaType.APPLICATION_JSON)
				.content("[ " + "{ \"question\": 1, \"selectedAnswer\": 2 },"
						+ "{ \"question\": 2, \"selectedAnswer\": 3 }," + "{ \"question\": 3, \"selectedAnswer\": 5 }"
						+ "]"))
				.andExpect(status().isOk()).andExpect(jsonPath("$." + TOTAL_QUESTIONS_TEXT, Matchers.equalTo(2)))
				.andExpect(jsonPath("$." + CORRECT_QUESTIONS_TEXT, Matchers.equalTo(2)));
	}
	...
	private void test_updateAnswers() throws Exception {
		...
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/1").param(ANSWER_TEXT_KEY, ANSWER_TEXT_1 + " updated"))
				.andExpect(status().isUnauthorized());

		// Update an Answer with invalid parameters
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/1").param(ANSWER_TEXT_KEY, "")
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isBadRequest());

		// Update an Answer that doesn't exist
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/55").param(ANSWER_TEXT_KEY, ANSWER_TEXT_1 + " updated")
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isNotFound());

		// Update an Answer from another user
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/1").param(ANSWER_TEXT_KEY, ANSWER_TEXT_1 + " updated")
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());
		...
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/updateAll").contentType(MediaType.APPLICATION_JSON)
				.content("[ " + "{ \"id\": 4, \"text\": \"asdf\" }" + "]").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isUnauthorized());

		// Update and re-order two answers on one go
		mvc.perform(post(AnswerController.ROOT_MAPPING + "/updateAll").contentType(MediaType.APPLICATION_JSON)
				.content("[ " + "{ \"id\": 2, \"text\": \"random2\" }," + "{ \"id\": 1, \"text\": \"random1\" }" + "]")
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isOk());

		mvc.perform(get(AnswerController.ROOT_MAPPING + "/1")).andExpect(status().isOk())
				.andExpect(jsonPath("$.text", Matchers.containsString("random1")))
				.andExpect(jsonPath("$.order", Matchers.is(2)));

		mvc.perform(get(AnswerController.ROOT_MAPPING + "/2")).andExpect(status().isOk())
	...
	private void test_fetchAnswers() throws Exception {
		...
		mvc.perform(get(AnswerController.ROOT_MAPPING + "/55")).andExpect(status().isNotFound());
		...
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/1/answers").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk()).andExpect(jsonPath("$", Matchers.hasSize(2)));

		// Get question 2's answers
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/2/answers").with(httpBasic(EMAIL_1, PASSWORD_1)))
	...
	private void test_createAnswers() throws Exception {
		...
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(1)))
				.andExpect(status().isUnauthorized());

		// Attempt to create an new Answer with wrong credentials
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(1))
				.with(httpBasic("Rand", PASSWORD_1))).andExpect(status().isUnauthorized());

		// Attempt to create an Answer for an inexistent quiz
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(55))
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isNotFound());

		// Attempt to question for an Answer that doesn't belong to the user/
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(1))
				.with(httpBasic(EMAIL_2, PASSWORD_2))).andExpect(status().isUnauthorized());
		...
		// Add three more answers (one per question) and check we can get them
		// through the Question Controller
		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_2)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_2).param(ANSWER_QUESTION_KEY, Integer.toString(1))
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isCreated());

		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(2))
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isCreated());

		mvc.perform(post(AnswerController.ROOT_MAPPING).param(ANSWER_TEXT_KEY, ANSWER_TEXT_1)
				.param(ANSWER_IS_CORRECT_KEY, ANSWER_IS_CORRECT_1).param(ANSWER_QUESTION_KEY, Integer.toString(3))
				.with(httpBasic(EMAIL_2, PASSWORD_2))).andExpect(status().isCreated());

		// Get the correct Answer for the first question
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/1/correctAnswer").with(httpBasic(EMAIL_1, PASSWORD_1)))
		...
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/1/correctAnswer").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk()).andExpect(jsonPath("$.id", Matchers.is(2)));
	}

	private void test_listAnswers_noAnswers() throws Exception {
		// List all Answers for an invalid Quiz
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/55/answers")).andExpect(status().isNotFound());

		// List all Answers for the first Quiz
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/2/answers")).andExpect(status().isOk())
				.andExpect(jsonPath("$", Matchers.hasSize(0)));
	}

	private void test_updateQuestions() throws Exception {
		// Update a question without being authenticated
		mvc.perform(post(QuestionController.ROOT_MAPPING + "/1").param(QUESTION_TEXT_KEY, QUESTION_TEXT_1 + " updated"))
				.andExpect(status().isUnauthorized());

		// Update a question with invalid parameters
		mvc.perform(post(QuestionController.ROOT_MAPPING + "/1").param(QUESTION_TEXT_KEY, "a")
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isBadRequest());

		// Update a question that doesn't exist
		mvc.perform(post(QuestionController.ROOT_MAPPING + "/55").param(QUESTION_TEXT_KEY, QUESTION_TEXT_1 + " updated")
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isNotFound());
		...
		mvc.perform(post(QuestionController.ROOT_MAPPING + "/updateAll").contentType(MediaType.APPLICATION_JSON)
				.content("[ " + "{ \"id\": 3, \"text\": \"asdf\" }" + "]").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isUnauthorized());
		...
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/1")).andExpect(status().isOk())
				.andExpect(jsonPath("$.text", Matchers.containsString("random1")))
				.andExpect(jsonPath("$.order", Matchers.is(2)));

		mvc.perform(get(QuestionController.ROOT_MAPPING + "/2")).andExpect(status().isOk())
				.andExpect(jsonPath("$.text", Matchers.containsString("random2")))
				.andExpect(jsonPath("$.order", Matchers.is(1)));
	}
	...
	private void test_fetchQuestions() throws Exception {
		...
		mvc.perform(get(QuestionController.ROOT_MAPPING + "/55")).andExpect(status().isNotFound());
		...
		mvc.perform(get(QuizController.ROOT_MAPPING + "/2/questions").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isOk()).andExpect(jsonPath("$", Matchers.hasSize(1)));
	}
	...
	private void test_createQuestions() throws Exception {
		...
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(1))).andExpect(status().isUnauthorized());

		// Attempt to create a new Quiz with wrong credentials
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(1)).with(httpBasic("Rand", PASSWORD_1)))
				.andExpect(status().isUnauthorized());

		// Attempt to create a question for an inexistent quiz
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(55)).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isNotFound());

		// Attempt to question for a quiz that doesn't belong to the user/
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(1)).with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());

		// Create a question for the first Quiz
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(1)).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isCreated());

		// Create a second question
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_2)
				.param(QUESTION_QUIZ_KEY, Integer.toString(1)).with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isCreated());

		// Create question with second user
		mvc.perform(post(QuestionController.ROOT_MAPPING).param(QUESTION_TEXT_KEY, QUESTION_TEXT_1)
				.param(QUESTION_QUIZ_KEY, Integer.toString(2)).with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isCreated());
	...
	private void test_listQuestions_noQuestions() throws Exception {
		...
		mvc.perform(get(QuizController.ROOT_MAPPING + "/55/questions").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isNotFound());
	...
	private void test_updatingQuizzes() throws Exception {
		...
		mvc.perform(post(QuizController.ROOT_MAPPING + "/2").param(QUIZ_NAME_KEY, QUIZ_NAME_1 + " updated")
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1 + " updated")).andExpect(status().isUnauthorized());

		// Update a quiz with invalid parameters
		mvc.perform(post(QuizController.ROOT_MAPPING + "/2").param(QUIZ_NAME_KEY, "a").param(QUIZ_DESCRIPTION_KEY, "a")
				.with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isBadRequest());

		// Update a quiz that doesn't exist
		mvc.perform(post(QuizController.ROOT_MAPPING + "/55").param(QUIZ_NAME_KEY, QUIZ_NAME_1 + " updated")
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1 + " updated").with(httpBasic(EMAIL_1, PASSWORD_1)))
				.andExpect(status().isNotFound());

		// Update a quiz from another user
		mvc.perform(post(QuizController.ROOT_MAPPING + "/1").param(QUIZ_NAME_KEY, QUIZ_NAME_1 + " updated")
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1 + " updated").with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());
		...
		mvc.perform(post(QuizController.ROOT_MAPPING + "/1").param(QUIZ_NAME_KEY, QUIZ_NAME_1 + " updated")
				...
				.andExpect(jsonPath("$." + QUIZ_DESCRIPTION_KEY,
						Matchers.containsString(QUIZ_DESCRIPTION_1 + " updated")));
	}

	private void test_fetchingQuizzes() throws Exception {
		// List all Quizzes without providing credentials
		mvc.perform(get(QuizController.ROOT_MAPPING)).andExpect(status().isOk())
				.andExpect(jsonPath("$.content", Matchers.hasSize(2)));

		// List all Quizzes providing credentials
		mvc.perform(get(QuizController.ROOT_MAPPING).with(httpBasic(EMAIL_1, PASSWORD_1))).andExpect(status().isOk())
				.andExpect(jsonPath("$.content", Matchers.hasSize(2)));

		// Search Quizzes
		mvc.perform(
				get(QuizController.ROOT_MAPPING + "/search?filter=" + QUIZ_NAME_1.substring(2, QUIZ_NAME_1.length())))
				.andExpect(status().isOk()).andExpect(jsonPath("$.content", Matchers.hasSize(1)));

		// Search Quizzes
		mvc.perform(get(QuizController.ROOT_MAPPING + "/search?filter=")).andExpect(status().isOk())
				.andExpect(jsonPath("$.content", Matchers.hasSize(2)));

		// Search Quizzes no results
		mvc.perform(get(QuizController.ROOT_MAPPING + "/search?filter=" + "testString")).andExpect(status().isOk())
		...
		mvc.perform(get(UserController.ROOT_MAPPING + "/55/quizzes")).andExpect(status().isNotFound());
		...
		mvc.perform(get(QuizController.ROOT_MAPPING + "/1")).andExpect(status().isOk())
				.andExpect(jsonPath("$.id", Matchers.is(1)));
	}
	...
	private void test_createQuiz_userEnabled(String continueRegistrationUser2Url) throws Exception {
		...
		mvc.perform(post(QuizController.ROOT_MAPPING).param(QUIZ_NAME_KEY, QUIZ_NAME_2)
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_2).with(httpBasic(EMAIL_2, PASSWORD_2)))
				.andExpect(status().isUnauthorized());

		// Activate the second user
		mvc.perform(get(continueRegistrationUser2Url)).andExpect(status().isOk());
	...
	private void test_createQuiz() throws Exception {
		...
		mvc.perform(post(QuizController.ROOT_MAPPING).param(QUIZ_NAME_KEY, QUIZ_NAME_1).param(QUIZ_DESCRIPTION_KEY,
				QUIZ_DESCRIPTION_1)).andExpect(status().isUnauthorized());

		// Attempt to create a new Quiz with wrong credentials
		mvc.perform(post(QuizController.ROOT_MAPPING).param(QUIZ_NAME_KEY, QUIZ_NAME_1)
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1).with(httpBasic("Rand", PASSWORD_1)))
				.andExpect(status().isUnauthorized());

		// Attempt to create a new Quiz with wrong parameters
		mvc.perform(post(QuizController.ROOT_MAPPING).param(QUIZ_NAME_KEY, "a")
				.param(QUIZ_DESCRIPTION_KEY, QUIZ_DESCRIPTION_1).with(httpBasic(EMAIL_1, PASSWORD_1)))
	...
	private void registerNewUser() throws Exception {
		...
		mvc.perform(post(UserController.ROOT_MAPPING + "/registration").param(USERNAME_KEY, USERNAME_2)
	...
	private void test_registerUser() throws Exception, IOException, MessagingException {
		...
		mvc.perform(post(UserController.ROOT_MAPPING + "/registration").param(USERNAME_KEY, USERNAME_1)
				.param(PASSWORD_KEY, PASSWORD_1_OLD).param(EMAIL_KEY, "aom")).andExpect(status().isBadRequest());

		// Create a new User
		mvc.perform(post(UserController.ROOT_MAPPING + "/registration").param(USERNAME_KEY, USERNAME_1)
				.param(PASSWORD_KEY, PASSWORD_1_OLD).param(EMAIL_KEY, EMAIL_1)).andExpect(status().isOk());
		...
		mvc.perform(get(continueRegistrationUrl + "33")).andExpect(status().isBadRequest());
